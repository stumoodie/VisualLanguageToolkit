create table CompoundEdge (id integer generated by default as identity (start with 1), model_id integer not null, edge_index integer not null, owning_node_id integer not null, constraint pk_CompoundEdge primary key (id), constraint xak1compoundedge unique (model_id, edge_index));
...
create table CompoundRootNode (id integer not null, referring_model_id integer not null, constraint xpkcompoundrootnode primary key (id));
create table CompoundChildNode (id integer not null, parent_node_id integer not null, constraint xpkcompoundchildnode primary key (id));
alter table RootFolder add constraint Relationship63 foreign key (repository_id) references Repository;
alter table MapDiagram add constraint Relationship666 foreign key (folder_id) references Folder;
...
alter table CompoundRootNode add constraint Relationship4 foreign key (id) references CompoundNode;
alter table CompoundChildNode add constraint Relationship5 foreign key (id) references CompoundNode;
alter table CompoundChildNode add constraint Relationship6 foreign key (parent_node_id) references CompoundNode;
alter table CompoundRootNode add constraint FKCompoundRo201502 foreign key (referring_model_id) references Model;
