...
create table BendPoint (id integer generated by default as identity (start with 1), creation_serial integer not null, index_pos integer not null, x_position double not null, y_position double not null, first_rel_dim_x double not null, first_rel_dim_y double not null, second_rel_dim_x double not null, second_rel_dim_y double not null, link_attribute_id integer, constraint pk_BendPoint primary key (id), constraint xak1BendPoint unique (link_attribute_id, creation_serial));
...
create table LinkTerminus (id integer not null, link_term_type smallint not null, offset_val double not null, end_dec_type integer not null, end_dec_width double not null, end_dec_height double not null, term_dec_type integer not null, term_dec_red integer not null, term_dec_green integer not null, term_dec_blue integer not null, term_dec_height double not null, term_dec_width double not null, owning_link_id integer, x_position double default 0 not null, y_position double default 0 not null, constraint pk_LinkTerminus primary key (id));
...
create table Property (id integer generated by default as identity (start with 1), canvas_attribute_id integer not null, displayed_label_id integer, name varchar(255) not null, constraint pk_Property primary key (id));
create table LabelAttribute (id integer not null, background_red integer not null, background_green integer not null, background_blue integer not null, y_position double not null, x_position double not null, width double not null, height double not null, constraint pk_Label primary key (id));
...
create table ShapeAttribute (id integer not null, name varchar(128) not null, name_vert_align integer default 2 not null, name_horiz_align integer default 5 not null, name_visible bit default 'true' not null, name_red integer not null, name_green integer not null, name_blue integer not null, object_type_id integer not null, description varchar(2048) not null, detailed_description longvarchar not null, fill_red integer not null, fill_green integer not null, fill_blue integer not null, line_red integer not null, line_green integer not null, line_blue integer not null, line_style integer not null, line_width integer not null, padding integer not null, shape_type smallint not null, x_position double not null, y_position double not null, height double not null, width double not null, url varchar(2048) not null, constraint pkShapeAttribute primary key (id));
...
alter table Property add constraint FKProperty207842 foreign key (displayed_label_id) references LabelAttribute (id);
