...
create table ObjectType (id integer generated by default as identity (start with 1), context_id integer not null, name varchar(128) not null, description varchar(2048) not null, classification_code integer not null, constraint pk_ObjectType primary key (id), constraint xak1objecttype unique (context_id, name));
create table Context (id integer generated by default as identity (start with 1), global_id varchar(256) not null, name varchar(128) not null, description varchar(2048) not null, major_version integer not null, minor_version integer not null, patch_version integer not null, constraint pk_Context primary key (id), constraint xak2Context unique (global_id), constraint xak1Context unique (name, major_version, minor_version, patch_version), unique (global_id), unique (name), unique (major_version), unique (minor_version), unique (patch_version));
...
create table LinkAttribute (id integer generated by default as identity (start with 1), canvas_id integer not null, creation_serial integer not null, object_type_id integer not null, source_terminus_id integer, target_terminus_id integer, name varchar(128) not null, detailed_description longvarchar not null, description varchar(2048) not null, line_red integer not null, line_green integer not null, line_blue integer not null, line_style integer not null, line_width integer not null, router_type smallint not null, url varchar(2048) not null, constraint pk_Link primary key (id), constraint xak1LinkAttribute unique (canvas_id, creation_serial));
