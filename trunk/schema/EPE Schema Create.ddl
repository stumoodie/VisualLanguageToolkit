...
create table LinkTerminus (id integer generated by default as identity (start with 1), owningLink integer, link_term_type smallint not null, offset_val smallint not null, end_dec_type integer not null, end_dec_width integer not null, end_dec_height integer not null, term_dec_type integer not null, term_dec_red integer not null, term_dec_green integer not null, term_dec_blue integer not null, term_dec_height integer not null, term_dec_width integer not null, constraint pk_LinkTerminus primary key (id), constraint xak1LinkTerminus unique (owningLink, link_term_type));
...
create table LinkAttribute (id integer generated by default as identity (start with 1), canvas_id integer not null, creation_serial integer not null, object_type_id integer not null, source_terminus_id integer not null, target_terminus_id integer not null, name varchar(128) not null, detailed_description longvarchar not null, description varchar(2048) not null, line_red integer not null, line_green integer not null, line_blue integer not null, line_style integer not null, line_width integer not null, router_type smallint not null, url varchar(2048) not null, constraint pk_Link primary key (id), constraint xak1LinkAttribute unique (canvas_id, creation_serial));
...
alter table LinkTerminus add constraint Relationship147 foreign key (owningLink) references LinkAttribute;
